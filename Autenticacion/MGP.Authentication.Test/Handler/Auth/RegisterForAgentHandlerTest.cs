...
public class RegisterForAgentHandlerTest
{

    private readonly Mock<IUserService> _registerServiceMock;
    private readonly Mock<IEmailService> _emailServiceMock;
    private readonly Mock<IValidator<RegisterByAgentRequest>> _validatorMock;
    private readonly Mock<IMapper> _mapperMock;
    private readonly MessageManager _messageManager;
    private readonly RegisterByAgentHandler _handler;

    public RegisterForAgentHandlerTest()
    {
        _registerServiceMock = new Mock<IUserService>();
        _validatorMock = new Mock<IValidator<RegisterByAgentRequest>>();
        ...
        _handler = new RegisterByAgentHandler(_registerServiceMock.Object, _validatorMock.Object, _mapperMock.Object, _messageManager);
    }
    ...
    public async Task Handle_ShouldReturnSuccess_WhenValidationSucceeds()
    {
        ...
        //1) Defino el request
        RegisterByAgentRequest registerRequest = CreateRegisterRequest();
        ...
        _registerServiceMock.Setup(s => s.RegisterByAgentAsync(userData)).ReturnsAsync(userId);
    ...
    public async Task Handle_ShouldReturnBadRequest_WhenParametersAreMissing()
    {
        ...
        {
            Errors = { new FluentValidation.Results.ValidationFailure("Email", _messageManager.GetNotification(FunctionalMessages.NOT_NULL, nameof(RegisterByAgentRequest.Email))) }
        };
    ...
    public async Task Handle_ShouldReturnConflict_WhenAlreadyExist()
    {
        ...
        _registerServiceMock.Setup(s => s.RegisterByAgentAsync(userData)).Throws(new DbUpdateException());
    ...
    public async Task Handle_ShouldReturnServiceUnavailable_WhenServiceUnavailable()
    {
        ...
        _registerServiceMock.Setup(s => s.RegisterByAgentAsync(userData)).Throws(new Exception());
    ...
    private RegisterByAgentRequest CreateRegisterRequest()
    {
        ...
        var registerRequest = new RegisterByAgentRequest
        {
            Email = "cristian.fer.mel@gmail.com",
            Person = personData,
        };
