...
namespace MGP.Discapacidad.Test.Handler.Agent.SUBE.Update;

public class UpdateTicketSubeHandlerTest
{
    private readonly Mock<ITicketService> _ticketServiceMock;
    private readonly Mock<IValidator<UpdateTicketSubeRequest>> _validatorMock;
    private readonly MessageManager _messageManager;
    private readonly Mock<IMapper> _mapperMock;
    private readonly UpdateTicketSubeHandler _handler;

    public UpdateTicketSubeHandlerTest()
    {
        _ticketServiceMock = new Mock<ITicketService>();
        _validatorMock = new Mock<IValidator<UpdateTicketSubeRequest>>();
        ...
        _handler = new UpdateTicketSubeHandler(
    ...
    {
        // Arrange
        var request = UpdateTicketSubeRequestObject();
        var updatedData = UpdateTickeSubeData();
        ...
        _mapperMock.Setup(m => m.Map<UpdateTicketSubeData>(request)).Returns(updatedData);

        _ticketServiceMock.Setup(ts => ts.UpdateTicketSubeAsync(updatedData)).Returns(Task.CompletedTask);
    ...
    public async Task Handle_Should_Throw_ParameterNotProviderException_When_Validation_Fails()
    {
        ...
        {
            Errors = [new ValidationFailure("TicketId", _messageManager.GetNotification(FunctionalMessages.NOT_NULL, nameof(UpdateTicketSubeRequest.TicketId)))]
        };
    ...
    public async Task Handle_Should_Return_NotFound_When_Ticket_Is_Not_Found()
    {
        // Arrange
        var request = UpdateTicketSubeRequestObject();
        var requestData = UpdateTickeSubeData();

        _validatorMock.Setup(v => v.ValidateAsync(request, It.IsAny<CancellationToken>())).ReturnsAsync(new ValidationResult());

        _mapperMock.Setup(m => m.Map<UpdateTicketSubeData>(request)).Returns(requestData);

        _ticketServiceMock.Setup(s => s.UpdateTicketSubeAsync(requestData)).Throws(new TicketNotFoundException());
    ...
    [Fact]
    public async Task Handle_Should_Return_TicketStatusNoFoundException_When_Status_Is_Not_Found()
    {
        // Arrange
        var request = UpdateTicketSubeRequestObject();
        var requestData = UpdateTickeSubeData();
        ...
        _ticketServiceMock.Setup(s => s.UpdateTicketSubeAsync(requestData)).Throws(new TicketStatusNotFoundException());
    ...
    public async Task Handle_Should_Return_InvalidDate_When_MedicalCommitteDateIsInvalid()
    {
        ...
        _ticketServiceMock.Setup(s => s.UpdateTicketSubeAsync(requestData)).Throws(new InvalidMedicalCommitteDateException());
    ...
    [Fact]
    public async Task Handle_Should_Return_ServiceUnavailable_When_ServiceIsUnavailable()
    {
        // Arrange
        var request = UpdateTicketSubeRequestObject();
        var requestData = UpdateTickeSubeData();
        ...
        _ticketServiceMock.Setup(s => s.UpdateTicketSubeAsync(requestData)).Throws(new Exception());
    ...
    private static UpdateTicketSubeRequest UpdateTicketSubeRequestObject()
    {
        return new UpdateTicketSubeRequest()
        {
            TicketId = Guid.Parse("96949bb8-8c14-466d-b34f-3c9d491dd96a"),
            CardPickupLocationsOptionsId = 1,
            TicketStatusId = 3,
    ...
    private static UpdateTicketSubeData UpdateTickeSubeData()
    {
        return new UpdateTicketSubeData()
        {
            TicketId = Guid.Parse("96949bb8-8c14-466d-b34f-3c9d491dd96a"),
            CardPickupLocationsOptionsId = 1,
            TicketStatusId = 3,
