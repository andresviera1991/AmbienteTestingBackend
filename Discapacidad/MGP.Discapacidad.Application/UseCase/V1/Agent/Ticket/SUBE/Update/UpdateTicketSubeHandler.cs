...
public class UpdateTicketSubeHandler : IRequestHandler<UpdateTicketSubeRequest, ResponseData<UpdateTicketSubeResponse, ResponseMessage>>
{
    private readonly ITicketService _TicketService;
    private readonly IValidator<UpdateTicketSubeRequest> _validator;
    private readonly MessageManager _messageManager;
    private readonly IMapper _mapper;

    public UpdateTicketSubeHandler(ITicketService TicketService,
                            IValidator<UpdateTicketSubeRequest> validator,
    ...
    public async Task<ResponseData<UpdateTicketSubeResponse, ResponseMessage>> Handle(UpdateTicketSubeRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await _TicketService.UpdateTicketSubeAsync(_mapper.Map<UpdateTicketSubeData>(request));

            var responseData = new ResponseData<UpdateTicketSubeResponse, ResponseMessage>(
                       code: (int)Code.SUCCESS,
                       data: new UpdateTicketSubeResponse() { Success = true },
                       message: null);
    ...
    private async Task<ResponseData<UpdateTicketSubeResponse, ResponseMessage>> HandleException(Code errorCode, List<string> message, UpdateTicketSubeResponse response = null)
    {
        var responseData = new ResponseData<UpdateTicketSubeResponse, ResponseMessage>(
                           code: (int)errorCode,
                           data: response,
